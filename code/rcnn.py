...
#   14    2    Valid accuracy: 0.274143     Test accuracy: 0.265004

model_folder = '../saved_models/rcnn_model/'
model_name = model_folder + 'rcnn_model.ckpt'
model_text = model_folder + 'rcnn_summary.txt'
...
output, _ = tf.nn.bidirectional_dynamic_rnn(fw_cell, bw_cell, embedded_statement, sequence_length=statement_len, dtype=tf.float32)
output = tf.concat([output[0], output[1]], axis=2)                                                  # [batch_size x max_statement_len x 2 * rnn_size]
output = tf.reshape(output, shape=[-1, max_statement_len, 2 * rnn_size, n_channels])                # [batch_size x max_statement_len x 2 * rnn_size x n_channels]

# convolutional and max-pool layer
pooled_outputs = []
for index, filter_size in enumerate(filter_sizes):
    W_conv = tf.get_variable('W_conv_{}'.format(index), shape=[filter_size, 2 * rnn_size, n_channels, n_filters], initializer=tf.contrib.layers.xavier_initializer())
    b_conv = tf.get_variable('b_conv_{}'.format(index), shape=[n_filters], initializer=tf.contrib.layers.xavier_initializer())

    conv_layer = tf.nn.relu(tf.add(tf.nn.conv2d(output, W_conv, strides=[1, 1, 1, 1], padding='VALID'), b_conv))
